...
class MainWindow(Screen):
	...
	def update_screen(self, update_assets=True, update_deposits=True):
		...
		assets_delta = self.model.fund.assets_profit + \
			self.model.fund.capital_in_assets
	...
	def set_table_deposits(self):
		...
		deposits_to_buy = [(name, x.time, x.perc, x.sum, x.time_left) 
										for name, x in self.model.market.deposits.items()]
		data_deposit = invested_deposits + deposits_to_buy

		self.table_deposit = MDDataTable(pos_hint={'center_x': 0.5, 'center_y': 0.5},
							#size_hint=(0.9, 0.6),
